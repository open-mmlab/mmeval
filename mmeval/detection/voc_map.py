...
class VOCMeanAP(BaseMetric):
    ...
    def _calculate_image_tpfp(
        ...
        ignore_gt_flags = np.concatenate((
            np.zeros((gt_bboxes.shape[0], 1), dtype=bool),
            np.ones((ignore_gt_bboxes.shape[0], 1), dtype=bool)
        ))
    ...
    def get_class_predictions(self, predictions: List[dict], class_index: int) -> List:
        ...
        Returns:
            list[np.ndarray]: predictions bboxes.
        """
        class_preds = []
        for pred in predictions:
            ...
            pred_bboxes_info = np.concatenate(
                (pred['bboxes'][pred_indices, :],
                 pred['scores'][pred_indices].reshape((-1, 1))), axis=1)
    ...
    def calculate_class_tpfp(self, predictions, groundtruths, class_index, pool):
        ...
        if pool is not None:
            ...
            for img_idx in range(len(class_preds)):
                tpfp = self._calculate_image_tpfp(
                    class_preds[img_idx],
                    class_gts[img_idx],
    ...
    def _format_metric_results(self, results_per_class: List[dict]) -> dict:
        ...
        for i, iou_thr in enumerate(self.iou_thrs):
            for j, area_range in enumerate(self._area_ranges):
                aps = [res['ap'][i][j] for res in results_per_class if res['num_gts'][i][j] > 0]
                if area_range == (None, None):
                    key = f'mAP@{iou_thr}'
                else:
                    key = f'mAP@{iou_thr}@{area_range}'
                eval_results[key] = np.mean(aps)
        return eval_results
